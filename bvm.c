...
void run(code init_c)
{
	...
	while (1) {
      ...
      s--;
		...
		case IFZ: {
        /* IfZ: tenemos en la stack un entero que es la
         * condición. El próximo opcode es la longitud
         * de la primer rama, en caso de que haya que
         * saltearla.
         */
      value condition = *--s;
      if (condition.i == 0)
...
int main(int argc, char **argv)
{
  ...
  /* Ya se puede cerrar el file descriptor */
  close(fd);
